...
// Formatter is the required interface for a custom logging record formatter.
...
// NewStringFormatter returns a new Formatter which outputs the logging record as a
// string based on the 'verbs' specified in the format string.
//
// The verbs:
//
// General:
//     %{id}        Sequence number for logging message (uint64).
//     %{pid}       Process id (int)
//     %{time}      Time when logging occurred (time.Time)
...
//     %{color}     ANSI color based on logging level
//
...
// reset it or else the color state will persist past your logging message.  e.g.,
...
func NewStringFormatter(format string) (Formatter, error) {
	...
	if matches == nil {
		return nil, errors.New("logger: invalid logging format: " + format)
	}
...
// possible to have different logging formats for different backends.
